...
public class CustomerControllerTest {
    ...
    public void shouldSignUpForValidRequest() throws Exception {
        ...
        createdCustomerEntity.setUuid(UUID.fromString(customerId));
    ...
    public void shouldLoginForValidRequest() throws Exception {
        ...
        final String customerId = UUID.randomUUID().toString();
        customerEntity.setUuid(UUID.fromString(customerId));
        createdCustomerAuthEntity.setCustomerEntity(customerEntity);

        when(authService.login("9090909090", "CorrectPassword"))
                .thenReturn(createdCustomerAuthEntity);

        mockMvc
                .perform(post("/customer/login")
                        .contentType(MediaType.APPLICATION_JSON_UTF8_VALUE)
                        .header("authorization", "Basic " + getEncoder()
                                .encodeToString("9090909090:CorrectPassword".getBytes())))
                .andExpect(status().isOk())
                .andExpect(jsonPath("id").value(customerId))
                .andExpect(header().exists("access-token"));
        verify(authService, times(1)).login("9090909090", "CorrectPassword");
    }
    ...
    public void shouldNotLoginIfNoDataPresentForGivenMobileNo() throws Exception {
        when(authService.login("123", "CorrectPassword"))
                .thenThrow(new AuthenticationFailedException("ATH-001", "This contact number does not exist"));
        mockMvc
                .perform(post("/customer/login")
                        .contentType(MediaType.APPLICATION_JSON_UTF8_VALUE)
                        .header("authorization", "Basic " + getEncoder()
                                .encodeToString("123:CorrectPassword".getBytes())))
                .andExpect(status().isUnauthorized())
                .andExpect(jsonPath("code").value("ATH-001"));
        verify(authService, times(1)).login("123", "CorrectPassword");
    }
    ...
    public void shouldNotLoginForWrongPassword() throws Exception {
        when(authService.login("9090909090", "IncorrectPassword"))
                .thenThrow(new AuthenticationFailedException("ATH-002", "Password failed"));
        mockMvc
                .perform(post("/customer/login")
                        .contentType(MediaType.APPLICATION_JSON_UTF8_VALUE)
                        .header("authorization", "Basic " + getEncoder()
                                .encodeToString("9090909090:IncorrectPassword".getBytes())))
                .andExpect(status().isUnauthorized())
                .andExpect(jsonPath("code").value("ATH-002"));
        verify(authService, times(1)).login("9090909090", "IncorrectPassword");
    }
    ...
    public void shouldLogoutForValidRequest() throws Exception {
        ...
        createdCustomerAuthEntity.setCustomerEntity(customerEntity);
        when(authService.logout("access-token")).thenReturn(createdCustomerAuthEntity);
    ...
    public void shouldNotLogoutWhenCustomerIsNotLoggedIn() throws Exception {
        ...
        verify(authService, times(1)).logout("auth");
    }
    ...
    public void shouldUpdateCustomerDetails() throws Exception {
        ...
        when(mockCustomerService.updateCustomer("first", "last", customerEntity))
        ...
        verify(mockCustomerService, times(1))
                .updateCustomer(anyString(), anyString(), customerEntity);
    }
    ...
    public void shouldNotUpdateCustomerDetailsIfFirstNameNotPresentInTheRequest() throws Exception {
        ...
        verify(mockCustomerService, times(0)).updateCustomer(anyString(), anyString(), any());
    }
    ...
    public void shouldUpdateCustomerPassword() throws Exception {
        ...
        when(mockCustomerService.getCustomer("auth")).thenReturn(customerEntity);
        when(mockCustomerService.updatePassword("oldPwd", "newPwd", customerEntity))
        ...
        verify(mockCustomerService, times(1)).getCustomer("auth");
        verify(mockCustomerService, times(1))
                .updatePassword("oldPwd", "newPwd", customerEntity);
    }
    ...
    public void shouldNotUpdateCustomerPasswordIfNewPasswordDoesNotFollowRecommendedPasswordFormat() throws Exception {
        ...
        verify(mockCustomerService, times(1)).getCustomer("auth");
        verify(mockCustomerService, times(1)).updatePassword("oldPwd",
                "newPwd", customerEntity);
    }
